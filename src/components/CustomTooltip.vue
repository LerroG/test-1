...
const procentValue = props.data[1].value ? `${Math.round((props.data[1].value / props.data[0].value) * 100)} %` : '0 %'
</script>
  ...
  <Card class="text-sm">
    ...
    <CardContent class="p-3 min-w-[180px] flex flex-col gap-1">
      <div v-for="(item, key) in data" :key="key" class="flex justify-between">
        ...
        <span class="font-semibold ml-4">{{ key === 0 ? item.value : procentValue }}</span>
      </div>
    </CardContent>
  </Card>
</template>
